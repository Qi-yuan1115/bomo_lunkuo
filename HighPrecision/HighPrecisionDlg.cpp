...
#define  Bomo_Chuli_lengthe 15  //每次处理的采样间隔0.002m
#define Bomo_length  (int)(Bomo_Chuli_lengthe/ Bomo_Caiyang_jiange)  //每次处理的长度

#define Bomo_Xianshi_Length 8//单次屏幕显示得波磨长度/m
#define Bomo_Xianshi_dum (int)(Bomo_Xianshi_Length/0.002) //单次屏幕显示得波磨长度


#define  Bomo_changbo_RMS_length 5  //每5m计算一次长波
#define  Bomo_zhongbo_RMS_length 1.5  //每1.5m计算一次中波
#define  Bomo_duanbo_RMS_length 0.5  //每0.5m计算一次短波
...
//文件保存标志
BOOL Bomo_Flag_Calculate_shuju_to_txt = FALSE;//计算结果报讯
...
CString PSD_strFileName_chaoxianbi;
...
CHighPrecisionDlg::CHighPrecisionDlg(CWnd* pParent /*=NULL*/)
	...
	, Bomo_paint_count_pre(0)
	, Bomo_paint_count_beh(0)
{
...
void CHighPrecisionDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHARTCTRL7, Bomo_ChartCtrl_R);
	DDX_Text(pDX, IDC_BMSTATA2, Bomo_caiji_zhuangtai);
	DDX_Text(pDX, IDC_BMSTATA1, Bomo_chuli_zhuangtai);
	DDX_Text(pDX, IDC_BMSTATA3, Bomo_paint_count_pre);
	DDX_Text(pDX, IDC_BMSTATA4, Bomo_paint_count_beh);
}
...
{	
	...
	/********读取定义********/
	float outlineX[SensorMaxPoint] = { 0 }, outlineY[SensorMaxPoint] = { 0 };
	//读入测试数据
	double currentDistance = 0.1;
	CString filePath = "K下-000000-000050M-前右原始轮廓点b.txt";
	...
	while (bFlagStartThread)
	{
		...
		/**************读取数据**********************/
		if (fMeasureFile.Open(filePath, CFile::modeRead))
		{
			int iNeedCount = 0;
			CString strLinetemp;
			bFinishFlag = false;
			...
			while (fMeasureFile.ReadString(strLinetemp))
			{
				...
				else
				{
					...
					if (currentDistance - 10 >= 0) {
						...
						nRtrValueGetPoint = iNeedCount;
						//nRtrValueGetPoint = ZLDS200GetPoint(x_results, z_results, &measureNum, &packetNum, &GetDataPointnum, TRUE, g_InitiChooseL); //获取数据
						//计算里程
						for (unsigned int it = 0; it < nRtrValueGetPoint; ++it)
						{

							x_results[it] = outlineX[it];
							z_results[it] = outlineY[it];
						}
						...
						iNeedCount = 0;
						outlineX[SensorMaxPoint] = { 0 }, outlineY[SensorMaxPoint] = { 0 };
...
{	
	...
	int ZeroGetPoint=0;//记录采集零个点的轮廓
	int nRtrValueGetPoint=0;
	//float x_results[SensorMaxPoint] = { 0 }, z_results[SensorMaxPoint] = { 0 };
	...
	//读入测试数据
	double currentDistance = 0.1;
	CString filePath = "K下-000000-000050M-前左原始轮廓点b.txt";
	CStdioFile fMeasureFile;
	bool bFinishFlag = false;
	/********读取定义********/
				...
				else
				{
					...
					if (currentDistance - 10 >= 0) {
						...
						for (unsigned int it = 0; it < nRtrValueGetPoint; ++it)
						{
							...
							x_resultsRight[it] = outlineX[it];
							z_resultsRight[it] = outlineY[it];
						}
...
{
	//m_ChartCtrl_LeftDown.ShowWindow(FALSE);//隐藏该控件
	m_ChartCtrl_LeftDown.ShowWindow(TRUE);//显示该控件
	m_ChartCtrl_RightDown.ShowWindow(TRUE);
	Bomo_ChartCtrl_R2.ShowWindow(FALSE);//隐藏该控件
	Bomo_ChartCtrl_R.ShowWindow(FALSE);
	...
	if (Bomo_Flag_bomo_Jisuan_R == TRUE)
	{
				...
				if (1)
				{
					...
					if (hDll2)
					{
						...
						pingshundu(Bomo_a_1_3Hz_signal_R, Bomo_a_3_10Hz_signal_R, Bomo_a_10_100Hz_signal_R, Bomo_length, Bomo_RMS_30_100mm_R, Bomo_RMS_100_300mm_R, Bomo_RMS_300_1000mm_R,
					...
					//Bomo_Flag_Calculate_shuju_to_txt = TRUE;

					Bomo_Calculate_shuju_to_txt();//保存数据
...
void CHighPrecisionDlg::Bomo_Calculate_shuju_to_txt()
{
...

				{
					...
					strTime_changbo.Format(("平顺度输出\\结果输出\\RMS值\\长波RMS值--%04d%02d%02d-%02d%02d%02d"), TCurrentTime.GetYear(), TCurrentTime.GetMonth(), TCurrentTime.GetDay(),
						TCurrentTime.GetHour(), TCurrentTime.GetMinute(), TCurrentTime.GetSecond());
					PSD_strFileName_changbo.Format(("%s.txt"), strTime_changbo);
					//中波RMS值结果名
					strTime_zhongbo.Format(("平顺度输出\\结果输出\\RMS值\\中波RMS值--%04d%02d%02d-%02d%02d%02d"), TCurrentTime.GetYear(), TCurrentTime.GetMonth(), TCurrentTime.GetDay(),
						TCurrentTime.GetHour(), TCurrentTime.GetMinute(), TCurrentTime.GetSecond());
					PSD_strFileName_zhongbo.Format(("%s.txt"), strTime_zhongbo);
					//短波RMS值结果名
					strTime_duanbo.Format(("平顺度输出\\结果输出\\RMS值\\短波RMS值--%04d%02d%02d-%02d%02d%02d"), TCurrentTime.GetYear(), TCurrentTime.GetMonth(), TCurrentTime.GetDay(),
						TCurrentTime.GetHour(), TCurrentTime.GetMinute(), TCurrentTime.GetSecond());
					PSD_strFileName_duanbo.Format(("%s.txt"), strTime_duanbo);
					//超先比
					strTimee_chaoxianbi.Format(("平顺度输出\\结果输出\\RMS值\\超限比--%04d%02d%02d-%02d%02d%02d"), TCurrentTime.GetYear(), TCurrentTime.GetMonth(), TCurrentTime.GetDay(),
						TCurrentTime.GetHour(), TCurrentTime.GetMinute(), TCurrentTime.GetSecond());
					PSD_strFileName_chaoxianbi.Format(("%s.txt"), strTimee_chaoxianbi);
				...
				FILE *fpz_changbo;
				fpz_changbo = fopen(PSD_strFileName_changbo, "a");
				//fpz22 = fopen("PSD_strFileName.txt", "a");
				//fp = fopen("平顺度.txt", "r");
				if (fpz_changbo == NULL)
				{
					printf("cannot open _changbo file!\n");
					//return 0;
				}

				FILE *fpz_zhongbo;
				fpz_zhongbo = fopen(PSD_strFileName_zhongbo, "a");
				//fpz22 = fopen("PSD_strFileName.txt", "a");
				//fp = fopen("平顺度.txt", "r");
				if (fpz_zhongbo == NULL)
				{
					printf("cannot open _zhongbo file!\n");
					//return 0;
				}

				FILE *fpz_duanbo;
				fpz_duanbo = fopen(PSD_strFileName_duanbo, "a");
				//fpz22 = fopen("PSD_strFileName.txt", "a");
				//fp = fopen("平顺度.txt", "r");
				if (fpz_duanbo == NULL)
				{
					printf("cannot open e_duanbofile!\n");
					//return 0;
				}


				FILE *fpz_chaoxianbi;
				fpz_chaoxianbi = fopen(PSD_strFileName_chaoxianbi, "a");
				//fpz22 = fopen("PSD_strFileName.txt", "a");
				//fp = fopen("平顺度.txt", "r");
				if (fpz_chaoxianbi == NULL)
				{
					printf("cannot open e_duanbofile!\n");
					//return 0;
				}
...
void CHighPrecisionDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == 0)
	{
		...
		else if (Bomo_huantu_vector_read_or_write_sensor_R1.size() >Bomo_length - 1)
		{
			...
			Bomo_paint_count_pre = Bomo_paint_count_R*Bomo_Chuli_lengthe;	//画图位置、
			Bomo_paint_count_beh = (Bomo_paint_count_R+1)*Bomo_Chuli_lengthe;	//画图位置
			UpdateData(FALSE); //内容更新到对应的控件
